...
\begin{equation}
	r_{i,j} = -l + 2lx_{i,j};\,i=1,2;\,j=1,2,3.
\end{equation}
$x_{i,j}$ is a uniformly distributed random number between 0 and 1. Thus $r_{i,j}$ goes from $-l$ to $l$. The Jacobi determinant for this is $(2l)^6$, as we have 6 variables. We solve the integral as it appears with $x_1,\,x_2,\,y_1,\,y_2,\,z_1,\,z_2$.
...
=======
	\frac{r_1^2r_2^2\sin\theta_1\sin\theta_2}{r_{12}},
\end{equation}
where $r_{12}$ is given in eq.~(\ref{eq:r12}). The PDF of $r_i$ is given by
\begin{equation}
...
<<<<<<< HEAD
The Jacobi determinant is in this case given by $(1/4)^2\times\pi^2\times(2\pi)^2 = \pi^4/4$.
=======
